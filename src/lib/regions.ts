// 대한민국 행정구역 (광역시·도 → 시·군·구).
// 데모용 정적 데이터. 추후 행정안전부 행정구역 API와 동기화 가능.

export interface ProvinceRegions {
  province: string;
  cities: string[];
}

export const REGION_TREE: ProvinceRegions[] = [
  {
    province: '서울특별시',
    cities: [
...
export const PROVINCE_OPTIONS = REGION_TREE.map((p) => p.province);

export const getCitiesByProvince = (province: string): string[] =>
  REGION_TREE.find((p) => p.province === province)?.cities ?? [];

export const getProvinceByCity = (city: string): string | null => {
  for (const p of REGION_TREE) {
