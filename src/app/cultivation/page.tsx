...
function CultivationInner() {
  ...
  const onSubmitSearch = () => {
    ...
    router.push(`/cultivation?q=${encodeURIComponent(next)}`);
  };

  const onSelectCrop = (c: CropOption) => {
    const qParam = q ? `q=${encodeURIComponent(q)}&` : '';
    router.push(
      `/cultivation?${qParam}itemCode=${c.item_code}&kindCode=${c.kind_code}`,
    );
  };
        ...
        <Stack gap="lg">
          ...
          <Box>
            ...
            <Text c="dimmed" size="sm" mt={6}>
              작목명을 입력해 보세요. 선택한 작목의 농촌진흥청 농업기술길잡이 e-book과 GPT 키포인트 요약을 표시합니다.
            </Text>
          </Box>
    ...
    >
      <Stack align="center" gap="xs">
        ...
        <Text size="sm" c="gray.7" ta="center" maw={420}>
          작목명을 입력하면 KAMIS 분류에서 일치하는 작목 목록이 표시되고, 선택하면 농사로에서 재배 지침과 GPT 키포인트 요약을 받아옵니다.
        </Text>
      </Stack>
    </Card>
  );
    ...
    <Stack gap="md">
      ...
      <Card radius="md" p="md" withBorder bg="white">
        <Group gap="xs" wrap="nowrap">
          ...
          <Text size="sm" c="gray.7">
            출처: 농사로 (신)작목별농업기술정보 / cropEbook (농촌진흥청). 본문 텍스트는 PDF·웹뷰어에서 확인하세요.
          </Text>
        </Group>
      </Card>
    </Stack>
